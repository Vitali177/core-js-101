...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  if (a > b) {
    return `${bracket1}${b}, ${a}${bracket2}`;
  }
  return `${bracket1}${a}, ${b}${bracket2}`;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (arrOpen.indexOf(str[i]) !== -1) {
      stack.push(str[i]);
    } else {
      const index = arrClose.indexOf(str[i]);
      if (stack[stack.length - 1] === arrOpen[index]) {
        stack.pop();
      } else {
        return false;
      }
...
function toNaryString(num, n) {
  ...
  while (decimalNumber !== 0) {
    result.unshift(decimalNumber % p);
    decimalNumber = Math.floor(decimalNumber / p);
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  const directoryes = [];
  let directory = pathesArr[0][0];
  let flag = true;

  pathesArr.forEach((item) => {
    if (item[0] !== directory) {
      flag = false;
    }
  });
